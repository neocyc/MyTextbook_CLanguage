...
//echo $private_key;
$pi_key =  openssl_pkey_get_private($private_key);//??函?可用?判?私?是否是可用的，可用返回?源id Resource id
$pu_key = openssl_pkey_get_public($public_key);//??函?可用?判?公?是否是可用的
...
$data = "This is a test!";//原始?据
$encrypted = ""; 
$decrypted = ""; 
 
openssl_private_encrypt($data,$encrypted,$pi_key);//私?加密
$encrypted = base64_encode($encrypted);//加密后的?容通常含有特殊字符，需要????下，在网??通?url???要注意base64??是否是url安全的
openssl_public_decrypt(base64_decode($encrypted),$decrypted,$pu_key);//私?加密的?容通?公?可用解密出?
 
openssl_public_encrypt($data,$encrypted2,$pu_key);//公?加密
$encrypted2 = base64_encode($encrypted2);
openssl_private_decrypt(base64_decode($encrypted2),$decrypted2,$pi_key);//私?解密
?>
<html>
<head>
...
<form name="testphp" action="urltest.php" method="get">
...
<?php
if($_GET["encode"]!="")
	openssl_private_decrypt(base64_decode($_GET["encode"]),$str1,$pi_key);//私?解密
echo $str1;
?>
</textarea><br/>
</body>
